...
/**
 * Collection of items created by a constructor. Items in the pool are created
 * on the fly as needed, and old items that have since been freed can be used
 * again.
 * 
 * Be sure to mark the object you get out of the pool as free when you are done
 * using it!
 */
var ItemPool = function(constructor) {
    this.pool = [];
    this.itemConstructor = constructor;
};

ItemPool.prototype.getFreeItem = function() {
    for (var k = 0; k < this.pool.length; k++) {
        if (this.pool[k].free) {
            this.pool[k].free = false;
            return this.pool[k];
        }
    }
    
    // There are no free items, create a new one
    var item = {
        index: this.pool.length,
        free: false,
        data: this.itemConstructor()
    };
